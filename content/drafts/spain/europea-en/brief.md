# Brief: Universidad Europea university card (EN)

- **URL:** `/study-in-spain/universidad-europea`
- **Type:** university-card
- **Audience:** applicants and parents from Pakistan, India, Nigeria, Bangladesh, Egypt, GCC; also graduates considering a master's and staying to work.
- **Fact sheets:** `content/facts/spain-europea.md` (university) + `content/facts/spain.md` (country: visa, documents, living). Do not read other drafts.

## Keywords

universidad europea — 74 000 / IN 720

## Structure

1. **Intro** (no H2): what the university is in 3 sentences + who it suits and who it doesn't.
2. **Key facts table** (✅ only).
3. **H2: Tuition fees** — table by program with price-list year; if no fees in the fact sheet, say honestly we confirm them for your program.
4. **H2: Programs and the degree you receive**.
5. **video slot:** "Universidad Europea campus walkthrough", 60–90 sec.
6. **H2: Admission requirements for international students** — processBlock: grades by curriculum, English, documents and certification, 2027 timeline (no invented dates).
7. **H2: What to know before you apply** — honest limitations.
8. **surveyBlock**
9. **H2: Rankings and accreditation** — ✅ only.
10. **H2: Housing and student life**.
11. **H2: Student visa** — short, link to the visa page.
12. **H2: Other options** — one paragraph with links.
13. **accordionBlock** — FAQ 6–8.
14. **cta** — we check your documents against Universidad Europea's requirements and compare 2–3 alternatives.

If the fact sheet says the university does not work with agencies, follow the system prompt rule.

## Allowed internal links (never link to the page itself)

`/study-in-spain`, `/study-in-spain/spain-student-visa`, `/study-in-spain/admission-requirements`, `/study-in-spain/masters-in-spain`, `/study-in-spain/study-and-work-in-spain`, `/study-in-spain/ucam`, `/study-in-spain/universidad-europea`, `/study-in-spain/ceu-san-pablo-university`, `/study-in-spain/complutense-university-of-madrid`, `/study-in-spain/university-of-barcelona`, `/contacts`
