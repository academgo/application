...
- `whatsapp-link.mjs --apply` — меняет ссылку на WhatsApp во всех документах
  Sanity (черновиках и опубликованных) на ту, что задана в скрипте.
- `remove-years.mjs content/years-edits --apply` — убирает годы, которые
  устаревают каждый год (учебные годы, наборы, годы прайсов). Правки лежат в
  `content/years-edits` парами «было → стало»; перед записью скрипт проверяет,
  что фрагмент не менялся и что не появилось новых чисел. Исторические годы,
  номера законов и сроки аккредитаций остаются.

После повторного импорта страниц годы вернутся из `content/drafts`: исходные
черновики в markdown не переписывались, источником текста теперь служит Sanity.
